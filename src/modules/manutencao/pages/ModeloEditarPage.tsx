...
import { updateEquipamentoModelo } from '../utils/equipamentoModelosApi';
...
export default function ModeloEditarPage() {
  ...
  if (!podeGerenciar) {
    return (
      <Card className="!p-5 text-sm text-[var(--color-fg-muted)]">
        Você não tem permissão para editar modelos.
      </Card>
    );
  }
  ...
  async function handleSubmit(data: EquipamentoModelo) {
    await updateEquipamentoModelo(id, data);
    await qc.invalidateQueries({ queryKey: ['manutencao'] });
    navigate(`/manutencao/modelos/${id}`);
  }
    ...
    <div className="flex flex-col gap-4 max-w-4xl">
      <nav aria-label="Breadcrumbs" className="flex items-center text-xs text-[var(--color-fg-muted)] gap-1">
        ...
        <Link to={`/manutencao/modelos/${id}`} className="hover:text-[var(--color-fg)]">
          {modelo.data.modeloNome}
        </Link>
        <ChevronRight aria-hidden className="w-3.5 h-3.5" />
        <span className="text-[var(--color-fg)] font-medium">Editar</span>
      </nav>

      <h2 className="text-lg font-semibold text-[var(--color-fg)]">Editar modelo</h2>
