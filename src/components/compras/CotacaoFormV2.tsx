...
import { buscarInsumosSimilares } from '../../utils/comprasValidator';
...
export default function CotacaoFormV2({
  ...
  onSubmit, onCancel, onCreateFornecedor, onGerarOC, onAbrirEnviar,
}: Props) {
  const { usuario } = useAuth();
  const { showToast } = useToast();
  void categorias; // categorias podem ser usadas no futuro pra filtrar
  ...
  useEffect(() => {
    ...
    if (pedido.itens.length > 0) {
      setItens(pedido.itens.map((i) => ({ ...i })));
    }
    if (pedido.descricaoLivre) {
      setDescricaoLivre(pedido.descricaoLivre);
    }
  }, [pedidoVinculadoId, initial, pedidosAprovados]);
  ...
  }, [cotFornecedores, itens, showToast]);
      ...
      <header className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <div className="sm:col-span-2 lg:col-span-1">
          ...
          <select
            ...
            onChange={(e) => setPedidoVinculadoId(e.target.value)}
...
function ItemRow({
  item, insumos, unidadesOptions, onChange, onRemove,
}: {
          ...
          <div className="absolute z-30 left-2 right-2 mt-1 rounded-lg border border-[var(--color-border)] bg-[var(--color-surface-1)] shadow-lg max-h-48 overflow-auto">
            {sugestoes.map((s) => {
              ...
              return (
                ...
                >
                  {s.nome} <span className="text-xs text-[var(--color-fg-subtle)] ml-1">({Math.round(s.similaridade * 100)}%)</span>
                </button>
              );
            })}
          </div>
