  ...
  if (filtrados.length === 0) {
    return (
      <div className="surface-raised p-8 text-center">
        <p className="text-[var(--color-fg-muted)]">Nenhum abastecimento de carreta encontrado.</p>
      </div>
    );
  }
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-[var(--color-border)] [&>tr:nth-child(even)]:bg-[var(--color-surface-2)]">
              {paginados.map((abast) => (
                <tr key={abast.id} className="hover:bg-[var(--color-accent-soft)] transition-colors">
