-- =============================================================================
-- ROLLBACK — COMB.FIFO.AUTOR (FIFO autoritativo no servidor)
-- =============================================================================
-- Reverte:
--   1. Triggers + trigger functions de recálculo
--   2. Função private.recompute_fifo_tanque
--   3. RPC registrar_saida_combustivel_fifo → versão de PRODUÇÃO anterior
--      (insere em consumos_lote + saidas_sem_suprimento)
--   4. Valores financeiros das saídas → backup capturado no fix
--
-- NÃO reverte consumos_lote / saidas_sem_suprimento (detalhamento derivado;
-- os recalculados pelo fix são mais corretos).
-- =============================================================================

-- 1. Triggers + funções de trigger
...
DROP FUNCTION IF EXISTS private.trg_recompute_fifo_saida();
DROP FUNCTION IF EXISTS private.trg_recompute_fifo_entrada();
DROP FUNCTION IF EXISTS private.trg_recompute_fifo_transferencia();

-- 2. Função de recálculo
DROP FUNCTION IF EXISTS private.recompute_fifo_tanque(text);

-- 3. RPC de produção anterior (insere saída + consumos_lote + saidas_sem_suprimento)
...
-- 4. Restaura valores financeiros das saídas a partir do backup
UPDATE public.saidas_combustivel s
SET preco_unitario              = b.preco_unitario,
    valor_total                 = b.valor_total,
    preco_medio_tanque_snapshot = b.preco_medio_tanque_snapshot
FROM private._fifo_backfill_backup_20260528 b
WHERE b.id = s.id;

-- Backup pode ser descartado após confirmar o rollback:
