-- Fix: registrar_saida_combustivel_fifo escrevia em saidas_lotes (view), mas
-- a view não popula consumo_tipo (NOT NULL) na tabela renomeada consumos_lote.
-- Resultado: "null value in column consumo_tipo of relation consumos_lote
-- violates not-null constraint".
--
-- Migration 20260524173325 renomeou saidas_lotes → consumos_lote e adicionou
-- consumo_tipo NOT NULL CHECK IN ('saida','transferencia_out','esvaziamento').
-- A view saidas_lotes filtra consumo_tipo='saida' mas não tem INSERT rule —
-- INSERT na view deixa consumo_tipo NULL e quebra.
--
-- Fix: insere direto em consumos_lote com consumo_id + consumo_tipo='saida'.
